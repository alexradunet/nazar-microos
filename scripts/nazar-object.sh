...
# nazar-object — CRUD tool for flat-file objects with YAML frontmatter.
#
# Object files are Markdown with YAML frontmatter stored under NAZAR_OBJECTS_DIR.
# Each object type gets its own subdirectory (e.g. journal/, task/, note/).
#
# Requires: yq (yq-go), jq
#
# Usage:
#   nazar-object create <type> <slug> [--field=value ...]
#   nazar-object read <type> <slug>
#   nazar-object list <type> [--status=X --project=Y --area=Z --tag=T]
#   nazar-object list --all [--status=X --project=Y --area=Z --tag=T]
#   nazar-object update <type> <slug> --field=value ...
#   nazar-object search <pattern>
#   nazar-object link <type/slug> <type/slug>
...
cmd_create() {
  ...
  [[ -z "$type" ]] && die "usage: nazar-object create <type> <slug> [--field=value ...]"
  [[ -z "$slug" ]] && die "usage: nazar-object create <type> <slug> [--field=value ...]"
...
cmd_read() {
  ...
  [[ -z "$type" ]] && die "usage: nazar-object read <type> <slug>"
  [[ -z "$slug" ]] && die "usage: nazar-object read <type> <slug>"
...
cmd_list() {
  ...
  [[ -z "$type" && "$list_all" -eq 0 ]] && die "usage: nazar-object list <type> [--status=X ...] or nazar-object list --all"
...
cmd_update() {
  ...
  [[ -z "$type" ]] && die "usage: nazar-object update <type> <slug> --field=value ..."
  [[ -z "$slug" ]] && die "usage: nazar-object update <type> <slug> --field=value ..."
  [[ $# -eq 0 ]] && die "usage: nazar-object update <type> <slug> --field=value ..."
...
cmd_search() {
  local pattern="${1:-}"
  [[ -z "$pattern" ]] && die "usage: nazar-object search <pattern>"
...
cmd_link() {
  ...
  [[ -z "$ref_a" || -z "$ref_b" ]] && die "usage: nazar-object link <type/slug> <type/slug>"
...
case "$cmd" in
  ...
  search)  cmd_search "$@" ;;
  link)    cmd_link "$@" ;;
  "")      die "usage: nazar-object <create|read|list|update|search|link> ..." ;;
