...
export class CapabilityExtractor {
  ...
  /** Extract /pibloom/capability.yaml from a container. Returns false if not present. */
  async extractManifest(
    ...
    const result = await this.executor.exec("podman", [
      "cp",
      `${containerName}:/pibloom/capability.yaml`,
      outputPath,
    ]);
    return result.exitCode === 0;
  }

  /** Extract /pibloom/skills/<name>/ from container to host skills dir. */
  async extractSkills(
    containerName: string,
    skills: string[],
    outputDir: string,
    ...
    for (const skill of skills) {
      await this.executor.exec("podman", [
        "cp",
        `${containerName}:/pibloom/skills/${skill}`,
        `${outputDir}/${skill}`,
      ]);
    }
  }
