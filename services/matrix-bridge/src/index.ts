/**
 * Nazar Matrix Bridge — matrix-bot-sdk → Pi print mode bridge.
 *
  ...
  try {
    const { stdout } = await execFileAsync(config.piCommand, ["-p", text], {
      ...
      env: {
        ...
        NAZAR_OBJECTS_DIR: config.objectsDir,
      },
...
async function main(): Promise<void> {
  ...
  if (!config.accessToken) {
    throw new Error("NAZAR_MATRIX_ACCESS_TOKEN is required");
  }
  ...
  for (const userId of config.allowedUsers) {
    if (!validateMatrixUserId(userId)) {
      throw new Error(
        `Invalid Matrix user ID format in NAZAR_MATRIX_ALLOWED_USERS: '${userId}' (expected @localpart:domain)`,
      );
    }
  }
  ...
  console.log("Nazar Matrix Bridge starting...");
